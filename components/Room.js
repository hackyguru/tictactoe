...
export default function Room(props) {
    ...
    let { messages: storeMessages } = useStoreMessages({
        node,
        decoder,
    });
    ...
    useEffect(() => {
        if (node !== undefined) {
            if (player == false) {
                const p = sessionStorage.getItem('roomId') == props.room && sessionStorage.getItem('player') == 'x' ? 'x' : 'o';
    ...
    if (nodeStart === false || player === false || opponentJoined === null) {
        return <Loading />
    }
    ...
    function renderBoxes() {
        ...
        for (let i = 1; i < 10; i += 1) {
            if (boxes[i] == undefined) {
                boxElements.push(<div onClick={() => handlePlay(i.toString())} className='w-20 h-20 border-2 border-white flex items-center justify-center text-2xl text-white' ></div>
                )
            } else {
                if (boxes[i] == 'x') {
                    boxElements.push(<div className='w-20 h-20 border-2 border-white flex items-center justify-center text-2xl text-white' >X</div>)
                } else {
                    boxElements.push(<div className='w-20 h-20 border-2 border-white flex items-center justify-center text-2xl text-white' >O</div>)
                }
            }
        }
        return (
            <div id='tiles' className="grid grid-cols-3 grid-rows-3 gap-3 mt-10">{boxElements}</div>
        )
    };
    ...
    return (
        <section id='room'>
            ...
            <div
                class="mx-auto max-w-screen-xl px-4 mt-20 lg:flex lg:h-mt-40 lg:items-center"
            >
                <div class="mx-auto max-w-xl">
                    ...
                    <div
                        class="px-3 py-3 text-center flex border-2 border-white text-white underline items-center justify-between"
                    >

                        <p>
                            http://localhost:3001/game/{props.room}
                        </p>
                    ...
                    <h1 class="text-3xl sm:text-4xl text-white mt-20">
                        {!opponentJoined  && "Waiting for opponent to join"}
                        {winner == null && opponentJoined && (Object.keys(boxes).length % 2 === 0 ? (player === 'x' ? "Your turn" : "Opponent's turn") : (player === 'o' ? "Your turn" : "Opponent's turn"))}
